...
const AirportComboBox = ({
  ...
  const filtered = query.trim()
    ? airports.filter((a) => a.toLowerCase().includes(query.toLowerCase()))
    : airports.slice(0, 50);

  const displayValue = (fullString) => {
    // if (mode === "prefix") {
  ...
  const handleSelect = (airport) => {
    const val = displayValue(airport);
    setQuery(val);
    onChange(val);
    setOpen(false);
  };
      ...
      {open && filtered.length > 0 && (
        <ul className="absolute z-50 mt-1 min-w-[250px] w-full max-h-56 overflow-y-auto bg-white border border-gray-300 rounded-xl shadow-lg text-sm">
          {filtered.map((airport, i) => (
            <li
              key={airport}
              className={`px-3 py-2 cursor-pointer ${i === highlighted ? "bg-blue-50 text-blue-500" : "hover:bg-gray-50"}`}
